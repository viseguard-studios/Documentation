...
\classitem{+arrive(mv:MovableEntity)}{Egy adott jármű érkezik az aszteroidára. Ha mv-nek (a paraméternek) AsteroidPlaces értéke Vessel, akkor beteszi mv-t a stationed-ba. Ha az Orbit, akkor hívja meg a AddBuilding(mv) metódust. A visited attribútuma értékét igazra állítja, és az összes szomszédjának (reachableAsteroids) a revealed értékét true-ra állítja (reveal metódust meghívja).}
...
\classitem{+mine():Item}{Akkor hívódik meg, amikor az aszteroidából akarnak nyersanyagot kibányászni. Ha az exploded attribútum éppen false, és a crustSize attribútum értéke 0, és az inventory attribútum getItems értéke nem null-t ad vissza, akkor meghívja az inventory removeItem(item) metódusát, és ezt az item objektumot visszatérési értékként adja vissza. Ha a metódus végén az inventory lista üres, akkor beállítja a mined érték true-ra. Más esetén nem csinál semmit, és null értéket adja vissza.}
...
\classitem{+placeItem(i:Item):bool}{Elem belehelyezése a raktárba, amennyiben mined értéke true (különben nem történik semmi). Meghívja az inventory attribútum tryInsertItem(i) metódusát, ha true értéket kap vissza, akkor hívja meg a insertItem(i) metódust, és true értéket ad vissza a metódus. Más esetben false-ot ad vissza. }
...
\classitem{+roundEnd()}{Az inventoryján meghívja a roundEnd() metódust.}
...
\classitem{+explode()}{Felrobban a raktár, el kell távolítani az összes elemet.(removeitem metódussal végigiterálva az items listán).} 
\classitem{+roundEnd()}{Végigiterál az itemeken, és meghívja a turnEnd() metódusukat.}
\end{class-template-method}
...
\classitem{-settlers:SpaceShip[1..*]}{A játék telepeseinek tárolója. }
\classitem{-scene:Scene}{Az adott játékhoz tartozó játéktér. }
\classitem{-engine:Engine}{Az adott program fő ablakja.}
\classitem{-neighboursList:Map<Asteroid,Asteroid>}{A pályán lévő aszteroida szomszédságok tárolója (a természetesen elérhető szomszédoké, nem a teleportkapun keresztül lévők)}
\end{class-template-attribute}
\begin{class-template-method}
...
\classitem{+removeSettler(s:SpaceShip)}{A paraméterként megadott objektumot eltávolítja a settlers attribútum listából, amennyiben benne van.}
...
\classitem{+turnEnd()}{Egy kör végén hívódik meg, visszaállítja az evaporated értéket false-ra.}
\end{class-template-method}
...
\end{class-template-baseclass}
\begin{class-template-attribute}
\classitem{-inventory[1]: IInventory}{Az aszteroidából még ki nem bányászott itemeket ebben az attribútumban gyűjtjük.}
\end{class-template-attribute}
\begin{class-template-method}
...
\classitem{+turnEnd()}{Egy kör végén végrehajtandó műveletek, alapértelmezetten üres.}
\end{class-template-method}
...
\classitem{+asteroidExploded()}{Az aktuális aszteroida felrobbanásakor hívódik meg. Meghívja az explode() függvényt, ha nincs máshogy felüldefiniálva.}
\end{class-template-method}
...
\classitem{+asteroidExploded()}{A robot aszteroidájának szomszédait lekéri (neighbours attribútum értéke), majd ezek közül egy véletlen algoritmussal választ egyet, ahova átkerül a robot. Meghívja a move(to) metódust, aminek paramétere a véletlenszerű aszteroida.}
\end{class-template-method}
...
\classitem{+removeEntity(e:Entity)}{Entitás eltávolítása az entities attribútumtömbből, robbanás esetén hívódik meg.}
...
\classitem{+explode()}{Felrobban a telepes (egy aszteroida robbanásának hatására). A scene attribútumának removeEntity(this) metódusát meghívja, ami eltávolítja őt az entitások közül. Az inventoryján is meghívja az explode() metódust, ami eltávolítja az összes elemet a raktárból. Továbbá a scenen keresztül elért gameManager (getManager()) settlers listájából is eltávolítja magát (removeSettler(this)).}
\end{class-template-method}
...
\classitem{+explode()}{Felrobban az UFO (egy aszteroida robbanásának hatására). A scene attribútumának removeEntity(this) metódusát meghívja, ami eltávolítja őt az entitások közül. Az inventoryján is meghívja az explode() metódust, ami eltávolítja az összes elemet a raktárból.}
\end{class-template-method}
...
\classitem{+explode()}{Felrobban  a jármű (egy aszteroida robbanásának hatására). A scene attribútumának removeEntity(this) metódusát meghívja, ami eltávolítja őt az entitások közül. }
...
A tesztesetek során a \verb">" jellel kezdődő sorok jelölik a játékos álltal beírt sorokat.
Minden más a program álltal visszaadott szöveg.
...
\begin{naplo}
	\naplotag{2021.03.31. 10:00 }{ 2 óra }{ Csapat }
	{ 
		Meeting 
	}
    \naplotag{2021.04.04. 20:00 }{ 1,5 óra }{ Csapat }
	{ 
		Feladatok szétosztása, megbeszélés
	}
	\naplotag{2021.04.07. 14:00 }{ 1 óra }{ Csapat }
	{ 
		Kérdések átbeszélése, haladás áttekintése
	}
    \naplotag{2021.04.10. 14:00 }{ 1 óra }{ csapat }
	{ 
		Meeting
	}
    \naplotag{2021.04.16. 18:00 }{ 3 óra }{ Dömötör Péter }
	{ 
		Class-ok átnézése.
	}
    \naplotag{2021.04.07. 16:00 }{ 2,5 óra }{ Nagy Beáta }
	{ 
		Osztályleírások
	}
	\naplotag{2021.04.11. 21:00 }{ 1,5 óra }{ Nagy Beáta }
	{ 
		Osztályleírások: formázás, kiegészítés
	}
	\naplotag{2021.04.12. 10:00 }{ 2 óra }{ Nagy Beáta }
	{ 
		Osztályleírások: kiegészítés, áttekintés
	}
    \naplotag{2021.04.03. 10:30 }{ 2 óra }{ Sike Ádám }
	{ 
		pályafájl formátum kitalálása
	}
    \naplotag{2021.04.10. 15:00 }{ 1 óra }{ Sike Ádám }
	{ 
		Ellenőrzések, kért példa teszteset elkészítése
	}
    \naplotag{2021.04.10. 18:00 }{ 1 óra }{ Dömötör Péter }
	{ 
		Bemeneti nyelv megírása
	}
    \naplotag{2021.04.11. 20:00 }{ 1 óra }{ Sike Ádám }
	{ 
		Apróbb ellenőrzések és hibajavítások 
	}
    \naplotag{2021.04.12. 11:00 }{ 1 óra }{ Sike Ádám }
	{ 
		Apróbb ellenőrzések és hibajavítások 
	}
    \naplotag{2021.04.10. 17:00 }{ 2 óra }{ Tatai Titusz }
    {
        FileOpener előkészítése
    }
    \naplotag{2021.04.12. 10:00 }{ 2 óra }{ Tatai Titusz }
    {
        Tesztesetek megírása
    } 
    \naplotag{2021.04.12. 10:00 }{ 2 óra }{ Dömötör Péter }
    {
        dokumentum ellenörzés
    } 
\end{naplo}
